...
// file name for csv download
  $filename = "Union_Accounts_Report_" . date('Ymd') . ".csv";
...
$delfd2s = Session::del('fd2s');
 ...
 header("Content-Type: application/vnd.ms-excel");
  ...
  if (!empty($fd2) && !empty($fd)){
   $result = mysql_query("SELECT AccountNo,COUNT(*) FROM uniontemp WHERE rdate BETWEEN '$fd' AND '$fd2' GROUP BY AccountNo") or die(mysql_error());
